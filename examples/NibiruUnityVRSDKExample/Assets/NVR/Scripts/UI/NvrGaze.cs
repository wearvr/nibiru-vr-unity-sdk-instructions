...
public class NvrGaze : MonoBehaviour {
  /// The active Gaze Pointer for this camera. Must have INvrGazePointer.
  /// The INvrGazePointer responds to events from this class.
  public GameObject PointerObject {
    ...
    set {
      if (value != null) {
        // Retrieve the INvrGazePointer component.
        var ptr = value.GetComponents<MonoBehaviour>()
            .Select(c => c as INvrGazePointer)
            .Where(c => c != null)
            .FirstOrDefault();
        ...
        if (ptr != null) {
          ...
          Debug.LogError("Object must have component which implements INvrGazePointer.");
        }
      } else {
        if (pointer != null) {
          if (isTriggered) {
  ...
  private INvrGazePointer pointer;
  ...
  private INvrGazeResponder currentTarget;
  ...
  private void HandleTrigger() {
    ...
    if (!isTriggered) {
      if (NvrViewer.Instance.Triggered || Input.GetMouseButtonDown(0)) {
        // Trigger started.
        isTriggered = true;
        if (pointer != null) {
          pointer.OnGazeTriggerStart(cam);
